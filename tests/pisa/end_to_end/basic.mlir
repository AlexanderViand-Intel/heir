...
// in test.bzl
//
// heir-opt --mlir-to-openfhe-bgv='entry-function=basic_test ciphertext-degree=4096' %s | bazel-bin/tools/heir-translate --emit-openfhe-pke

!t = tensor<4096xi16>

func.func @basic_test(%x: !t {secret.secret}, %y: !t {secret.secret}) -> !t {
  %r0 = tensor.empty() : !t
  %r = affine.for %i = 0 to 4096 iter_args(%r = %r0) -> !t {
